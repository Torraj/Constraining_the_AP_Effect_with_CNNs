...
def galaxyPos_NFW(dat, N_cen, N_sat, vel_bias, params = None):
    ''' SAMPLE POSITION AND VELOCITES. 
        GALAXIES ARE DRAWN FROM NFW Profile AND VELOCITIES FROM A GAUSSIAN DISTRIBUTION
...
# radial satellites positions:
    ...
    sigmav  = vel_bias * np.sqrt(4.3009e-9*mass_vir/r_vir)  # virial velocity in units [km/s], with G = 4.3009e-9
...
###
def drawnFromSnap(totsat,sat_len, sat_indx, r_vir, cen_pos, cen_vel, group_len, vec_DM, vel_bias):
    ''' GIVEN A SNAPSHOT OF DM PARTICLES DRAWN SATELLITES POSITION AND VELOCITIES
    ...
    for i in prange(N_cen):
        ...
        #For every satellite galaxy in a particular halo, we assign a velocity based on the velocities of dark matter
        #particles in that halo. Here, we add a factor alpha by which the galaxy orbital velocity will be different 
        #from the underlying dark matter particle orbital velocities. 
        vel_temp[fill_from:fill_to] = vel_bias * (vecDM_slice[ind,4:7] - cen_vel) + cen_vel

    return pos_temp, vel_temp


def galaxyPos_SNAP(dat, N_cen, N_sat, vel_bias, params = None):
    ''' SAMPLE POSITION AND VELOCITES FROM A SNAPSHOT OF DM PARTICLES
    ...
    masked_cen_pos = cen_pos[mask]   #[Mpc/h]
    ###
    #masked_cen_vel = cen_vel[mask]   #[km/s]
    group_len = group_len[mask].astype(int)
    ...
    ###
    sat_pos, sat_vel = drawnFromSnap(totsat,masked_sat,sat_indx,r_vir,masked_cen_pos,masked_cen_vel,group_len,vec_DM,vel_bias)
...
def sample_catalog(halos, centrals, satellites, vel_bias, params=None):
    ''' SAMPLE POSITION AND VELOCITES FROM A SNAPSHOT OF DM PARTICLES  
    ...
    if params['method'] == 'NFW':
        print('Doing NFW')
        galaxy_pos, galaxy_vel, galaxy_type, halo_indx = galaxyPos_NFW(halos, centrals, satellites, vel_bias, params)
    ...
    if params['method'] == 'SNAP':
        print('Doing SNAP')
        ###
        galaxy_pos, galaxy_vel, galaxy_type, halo_indx = galaxyPos_SNAP(halos, centrals, satellites, vel_bias, params)
